...
logger.info("Compatible API Configuration:", {
  ...
  config: {
    ...
    COMPATIBLE_API_KEY: process.env.COMPATIBLE_API_KEY
      ? "***" + process.env.COMPATIBLE_API_KEY.slice(-4)
      : undefined,
...
if (process.env.COMPATIBLE_API_BASE_URL && !process.env.COMPATIBLE_API_KEY) {
  logger.error(
    "COMPATIBLE_API_KEY is missing but COMPATIBLE_API_BASE_URL is set",
    {
      module: "generic-ai",
      method: "initialization",
    },
  ...
  throw new Error(
    "COMPATIBLE_API_KEY is required when COMPATIBLE_API_BASE_URL is set",
  );
}
